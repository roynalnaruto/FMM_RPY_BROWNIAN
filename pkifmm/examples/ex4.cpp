...
/* 
	 This is a more complex example, in which an RK4 scheme is used to
	 advance particle positions for a Stokesian simulation.  It uses the
	 Stokes kernel, repartitions at every time step.
 */
...
/* ************************************************** */
/* this function evaluates the potential and it uses the FMM evaluation */
#undef __FUNCT__
#define __FUNCT__ "RK4_eval_rhs"
int RK4_eval_rhs(Vec & x, Vec & ids, Vec & k1, Kernel3d_MPI & knl, double eps, bool redistr )
// k1 is created inside, so should be destroyed outside if redistr is
// true, "x" is redistributed; otherwise not; in any case layout of k1
// will be compatible with that of "x"
{
  ...
  if (redistr) {xx=x;	}
  else{ VecDuplicate(x,&xx);VecCopy(x, xx);	}
  ...
  // shift and scale points to  [0,1]^3 
...
int main(int argc, char** argv)
{
  ...
  PetscInt lclnumsrc;
	...
	if (!mpirank)	cout<<"Interpreting -numsrc as per-processor"<<endl;
	lclnumsrc = numsrc;
  ...
  PetscInt num_iter;
  double h;
  double eps;  // (particle size), computed by volume ratio
  double vol_ratio;
  PetscInt write_fraction;
  PetscTruth preserve_order;

  PetscOptionsGetReal(0,"-time_step",&h,&flg);  pA(flg);
	PetscOptionsGetReal(0,"-volume_ratio",&vol_ratio,&flg); pA(flg);
  ...
  for (int i=1; i<=num_iter; i++)
		{
			...
			RK4_eval_rhs(x, ids, k1, knl, eps, !preserve_order);   // VecCreate(k1) is called inside, so destroy k1 outside
			...
			VecCopy(x, xx);
			VecAXPY(xx, h/2,k1);
			RK4_eval_rhs(xx,work_vec, k2, knl, eps, false);   // k2 will have the layout of x
			...
			VecCopy(x, xx);
			VecAXPY(xx,h/2,k2);
			RK4_eval_rhs(xx,work_vec, k3, knl, eps, false);   // k3 will have the layout of x
			...
			VecCopy(x, xx);
			VecAXPY(xx,h,k3);
			RK4_eval_rhs(xx,work_vec, k4, knl, eps, false);   // k4 will have the layout of x
