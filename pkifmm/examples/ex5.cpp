...
int main(int argc, char** argv)
{
  ...
  int mpirank;  MPI_Comm_rank(comm, &mpirank);
	...
	ifstream file;
	file.open ("pos.txt");
	for(PetscInt k=0; k<3*lclnumsrc; k++){
		file>>srcPosarr[k];
	}
	file.close();
	...
	PetscInt kt=541; //LAPLACE KERNEL that returns potential and its gradient(and takes dipoles as input).
