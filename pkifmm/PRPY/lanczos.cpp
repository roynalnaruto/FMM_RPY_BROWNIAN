...
*/
                ...
                if(mobtype == KIFMM)
                {
					...
					computeRpy(nposfmm, sourcepos, force, radii, output, temp_output);
					postCorrectionAll(nposfmm, sourcepos, radii, numpairs_p, pairs, force, output); 

                    //Copy output back from temporary.
					for(tempi = 0; tempi<nposfmm*3; tempi++){
						w[tempi] = output[tempi].dr;
					}
				}
