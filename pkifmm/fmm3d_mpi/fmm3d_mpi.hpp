...
protected:
  ...
  // this kernel is used for source-to-check_surface translations  
  Kernel3d_MPI _knl_s2c;
  
  // this kernel is used for equivalent_surface-to-target translations  
  Kernel3d_MPI _knl_e2t;
...
public:
  ...
  int evaluate_upward();
  int evaluate_downward();
  int evaluate_ulist();
  int evaluate_vlist();
  int evaluate_wlist();
  int evaluate_xlist();

  // reduce-broadcast of upward densities
  int EquDenReduceBcast();
...
protected:
  ...
  int SrcEqu2TrgChk_dgemv(const DblNumMat& srcPos, const DblNumMat& srcNor, const DblNumMat& trgPos, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense);
  /*! Source Equivalent to Upward Check Multiplication */
  int SrcEqu2UpwChk_dgemv(const DblNumMat& srcPos, const DblNumMat& srcNor, Point3 trgctr, double trgrad, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense);
  /*! Source Equivalent to Downward Check Mutiplcation */
  int SrcEqu2DwnChk_dgemv(const DblNumMat& srcPos, const DblNumMat& srcNor, Point3 trgctr, double trgrad, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense);
  /*! Downward Equivalent to Target Check Mutiplcation */
  int DwnEqu2TrgChk_dgemv(Point3 srcctr, double srcrad, const DblNumMat& trgPos, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense);
  /*! Upward Equivalent to Target Check Multiplcation */
  int UpwEqu2TrgChk_dgemv(Point3 srcctr, double srcrad, const DblNumMat& trgPos, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense);
  //contributor data
