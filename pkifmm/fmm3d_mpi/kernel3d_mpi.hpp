...
enum {
  ...
  KNL_LAP_I   = 191, 
  // Laplace potential-with-derivatives kernel; source density is 1 number, target potential is 4 numbers: actual potential followed by three components of the gradient of the potential 
  KNL_LAP_S_UG   = 141, 
  // Lapclace potential-with-derivatives kernel-with-dipole-moments	
  KNL_LAP_S_UG_WD = 541,
  // RPY tensor : Part 1
  KNL_RPY = 431,
  // RPY tensor : Part 2
  ...
  /*! Levi-Civita Tensor */
  KNL_STK_E   = 392, 
  // stokes single-layer velocity plus gradient
  KNL_STK_S_UG = 341,
  // stokes FMM kernel plus gradient
  KNL_STK_F_UG = 351,
...
public:
  ...
  // direct density-to-potential calculation (without forming interaction matrix)
  int density2potential(const DblNumMat& srcPos, const DblNumVec& srcDen, const DblNumMat& srcNor, const DblNumMat& trgPos, DblNumVec& trgVal);
};
