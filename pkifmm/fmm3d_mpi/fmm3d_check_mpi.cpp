...
int FMM3d_MPI::check(Vec srcDen, Vec trgVal, PetscInt numChk, double& rerr)
{
  ...
  if (slclnum>0) // if there are some local sources ...
  {
    for(PetscInt i=0; i<ttlChk; i++) {
      ...
      SrcEqu2TrgChk_dgemv(lclSrcPosMat, lclSrcNorMat, oneChkPosMat, lclSrcDenVec, oneChkDenVec,1); // "1" means don't build interactiion matrix, evaluate directly ("fuse_dense")
    }
  }
