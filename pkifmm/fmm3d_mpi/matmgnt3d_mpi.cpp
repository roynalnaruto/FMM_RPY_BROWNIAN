...
// ---------------------------------------------------------------------- 
MatMgnt3d_MPI::MatMgnt3d_MPI(): _np(6), built_density_forplan(false), built_density_invplan(false), built_operator_forplan(false)
{
}
// ---------------------------------------------------------------------- 
MatMgnt3d_MPI::~MatMgnt3d_MPI()
{
  if (built_density_forplan)
    fftw_destroy_plan(density_forplan);  
  if (built_density_invplan)
    fftw_destroy_plan(density_invplan);  
  if(built_operator_forplan)
    fftw_destroy_plan(operator_forplan);
}
...
// ---------------------------------------------------------------------- 
int MatMgnt3d_MPI::UpwChk2UpwEqu_mat(int l)
{
...
int MatMgnt3d_MPI::UpwChk2UpwEqu_dgemv(int l, const DblNumVec& chk, DblNumVec& den)
{
  ...
  if(_homogeneous==true) {
	 ...
	 DblNumVec tmpDen(srcDOF*samPos(UE).n(), true, _wsbuf);	 clear(tmpDen);
	 iC( dgemv(1.0, _UpwChk2UpwEqu, chk, 1.0, tmpDen) );
	 //scale
	 ...
	 iC( dgemv(1.0, _UpwChk2UpwEqu, chk, 1.0, den) );
  }
  return (0);
}
// ---------------------------------------------------------------------- 
...
int MatMgnt3d_MPI::plnDen2EffDen(int l, const DblNumVec& plnDen, DblNumVec& effDen)
{
  ...
  if (!built_density_forplan)
  {
    density_forplan = fftw_plan_many_dft_r2c(3,nnn,srcDOF(), regDen.data(),NULL, srcDOF(),1, (fftw_complex*)(effDen.data()),NULL, srcDOF(),1, FFTW_ESTIMATE);
    built_density_forplan=true;
  }

  fftw_execute_dft_r2c(density_forplan, regDen.data(), (fftw_complex*)(effDen.data()));
...
int MatMgnt3d_MPI::effVal2PlnVal(int level, const DblNumVec& effVal, DblNumVec& plnVal)
{
  ...
  if(!built_density_invplan)
  {
    density_invplan = fftw_plan_many_dft_c2r(3,nnn,trgDOF(), (fftw_complex*)(effVal.data()),NULL, trgDOF(),1, regVal.data(),NULL, trgDOF(),1, FFTW_ESTIMATE);
    built_density_invplan=true;
  }

  fftw_execute_dft_c2r(density_invplan, (fftw_complex*)(effVal.data()), regVal.data());
...
// ---------------------------------------------------------------------- 
int MatMgnt3d_MPI::UpwEqu2DwnChk_mat(int l)
{
  ...
  if(_UpwEqu2DwnChk.m()==0)  {
      _UpwEqu2DwnChk.resize(7,7,7,-3,-3,-3);
      for (int i1=-3; i1<=3; i1++)
          for (int i2=-3; i2<=3; i2++)
			...
			nnn[1] = 2*_np;
                        ...
                        if (!built_operator_forplan)
                        {   
                            operator_forplan = fftw_plan_many_dft_r2c(3,nnn,srcDOF*trgDOF, tmp.data(),NULL, srcDOF*trgDOF,1, (fftw_complex*)(_UpwEqu2DwnChkii.data()),NULL, srcDOF*trgDOF,1, FFTW_ESTIMATE);
                            built_operator_forplan=true;
                        }
                        fftw_execute_dft_r2c(operator_forplan, tmp.data(), (fftw_complex*)(_UpwEqu2DwnChkii.data()));
                    }
                }
  }
  return 0;
