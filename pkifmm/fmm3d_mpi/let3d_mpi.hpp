...
public:
  ...
  enum merge_source {QUEUE, RECVD, EXTRA} ;
  enum merge_type {INSERT, ADD, DROP} ;
  struct reduce_bcast_data
  {
    vector<int> shared_octant; // indices of shared octants in "contributor" vector; desities of octants (in this order) should be packed initially
    vector <vector<int> > to_send ; // one vector per iteration; the vector contains indices of octants in the queue which are to be sent to partner;
    vector<int> recv_sizes;  // number of octants to be received at each iteration
    vector<int> extra_sizes;  // number of octants to be recvd from unpaired process at each iteration
    vector <vector<merge_source> > merging_srcs; // vector per iteration; entry "i" of the vector says what source to use at step "i" of merging:  queue, received octants or "extra" octants 
    vector <vector<merge_type> > merging_types; // vector per iteration; entry "i" of the vector says how to act at step "i" of merging: add or insert (push_back) or drop (discard) corresponding density
    vector <int> usr_octants; // after communication, this array gives index in "user" array where each octant in the final queue should go; NOTE:  some indices may be -1, so that the corresponding octant is discarded
    size_t max_queue_size;
    size_t max_to_send_size;
    size_t max_recvd_size;
    size_t max_extra_size;
  } ;
...
public:
  ...
  // pattern for reduce-broadcast
  struct reduce_bcast_data comm_pattern;
