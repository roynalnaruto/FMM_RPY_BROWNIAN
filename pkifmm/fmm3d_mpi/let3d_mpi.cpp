...
int Let3d_MPI::setup()
{
 ...
 // calculate displacements and total tree  size, reserve memory
  ...
  // save info into variables of Let3d_MPI class
  max_leaf_level = globMaxLevel;
  min_leaf_level = globMinLevel;
  ...
  linOct.clear(); //we no longer need our portion of leaves
	...
	/* Finalize LET:
		 1. assign source points to LET leaf boxes 
		 2. communicate ghost octants for all lists among processors
	*/
  pC( srcData(true /* do print statistitics */ ) );  
	/* 
		 1. assign target poitns to LET leaf boxes 
		 2. crate V,X,U,W list for each LET box.
	 */
  pC( trgData() );  
...
/**
 * @author Ilya Lashuk
 * @brief This function tells whether ``donut-shaped insulation layer'' of given octant "oct" overlaps with the part of the cube controlled by the processes with the ranks "r1" ... "r2" (inclusively)
 * 
 */
bool Let3d_MPI::ParInsLayerIntersectsRange(ot::TreeNode oct, int r1, int r2)
{
  ...
  vector<ot::TreeNode> v_list;
  v_list.reserve(189);

  for (size_t i=0; i<neighbs.size(); i++)
    if (neighbs[i]!=root_oct)  //this  means corresponding neighb. does not exist
      neighbs[i].addChildren(v_list);

  int dist = 2<<(maxLevel()-oct.getLevel());
  for (size_t i=0; i<v_list.size(); i++)
  {
    // skip if touching
    if (abs(int(v_list[i].getX()-oct.getX()))<dist && abs(int(v_list[i].getY()-oct.getY()))<dist && abs(int(v_list[i].getZ()-oct.getZ()))<dist)
      continue;

    if (v_list[i].getDLD()>=procMins[r1].getDFD()  && (r2==mpiSize()-1 || v_list[i].getDFD()<procMins[r2+1].getDFD() ))
      return true;
  }
...
/**
 * @author Ilya Lashuk
 * @brief This function implemements the exchange of octants between processes, so that for each octant the octant's geometrical data is sent from its contributors to users of its equivalent density. This routine also sets up the communication patterns for exchange of equivalent densities.
 * 
 * Input:
 *   Local tree at each process (local tree is accessed as "Let3d_MPI"-class field)
 * Output:
 *   At each process, all octants (from global tree) whos equivalent density the process uses are added to local tree.
 *  A structure of type "reduce_bcast_data" is created at each process.  This structure contains description of communication pattern for the exchange of equivalent densities during the evaluation phase. The structure is a field in "Let3d_MPI" class.
 */

#undef __FUNCT__
#define __FUNCT__ "Let3d_MPI::OctantsReduceBcast"
int Let3d_MPI::OctantsReduceBcast()
{
  PetscLogEventBegin(reduce_broadcast_bookkeeping,0,0,0,0);
  using namespace std;
  comm_pattern.shared_octant.clear();
  comm_pattern.to_send.clear();
  comm_pattern.recv_sizes.clear();
  comm_pattern.extra_sizes.clear();
  comm_pattern.merging_srcs.clear();
  comm_pattern.merging_types.clear();
  comm_pattern.usr_octants.clear();

  comm_pattern.max_queue_size=0;
  comm_pattern.max_to_send_size=0;
  comm_pattern.max_recvd_size=0;
  comm_pattern.max_extra_size=0;
  ...
  // make a list of ``local shared octants'';  that is octants that this process owns and which (octants) have donut-shaped insulation layer intersecting with areas controlled by other processors
  for(size_t q=0; q<_nodeVec.size(); q++)
    if (_nodeVec[q].tag() & LET_CBTRNODE)
    {
      unsigned lev=_nodeVec[q].depth();
      unsigned x,y,z;
      x = (_nodeVec[q].path2Node())(0) << maxLevel()-lev;
      y = (_nodeVec[q].path2Node())(1) << maxLevel()-lev;
      z = (_nodeVec[q].path2Node())(2) << maxLevel()-lev;

      ot::TreeNode oct (x,y,z,lev,3/*dim*/,maxLevel());
      if (mpiRank()>0 && ParInsLayerIntersectsRange(oct,0,mpiRank()-1) || mpiRank()<mpiSize()-1 && ParInsLayerIntersectsRange(oct,mpiRank()+1,mpiSize()-1))
      {
	oct.setWeight(q);
	l->push_back(oct);
      }
  ...
  for (int i=0; i<l->size(); i++)
    comm_pattern.shared_octant.push_back((*l)[i].getWeight());
  ...
  while (true)
  {
    ...
    int r1 = mpiRank()<=split? split+1 : first;
    int r2 = mpiRank()<=split? last : split;
    int q1 = mpiRank()<=split? first : split+1;
    int q2 = mpiRank()<=split? split : last;

    // build the list of octants to send to partner
    vector<ot::TreeNode> to_send;
    comm_pattern.to_send.push_back(vector<int>());
    for(size_t q=0; q<ll.size(); q++)
    {
      ...
      {
	comm_pattern.to_send.back().push_back(q);
    ...
    comm_pattern.max_to_send_size = max(int(comm_pattern.max_to_send_size),send_size);

    int recv_size=0; // note that for odd-sized partition, rank "split" will receive nothing, only send
    MPI_Status status;
    ...
    {
      assert(partner==last+1 && mpiRank()==split);
      MPI_Send(&send_size, 1, MPI_INT, last, 0, mpiComm());
    }

    comm_pattern.recv_sizes.push_back(recv_size);
    comm_pattern.max_recvd_size = max(int(comm_pattern.max_recvd_size),recv_size);
    vector<ot::TreeNode> recvd(recv_size);
    // exchange octants (geometrical info)
    if (partner<=last) // this rank has a pair
      ...
      MPI_Send(send_size? &to_send[0]:0, send_size, mpi_treenode, last, 0, mpiComm());

    // for odd-sized partitions, last rank in partition will receive extra octants from rank "split"
    vector<ot::TreeNode> extra;
    int extra_size=0;
    if ( (last-first+1)%2 &&  mpiRank()==last) // we are to receive "extra" octants from rank "split"
    {
      MPI_Recv(&extra_size, 1, MPI_INT, split, 0, mpiComm(), &status);
      extra.resize(extra_size);
      MPI_Recv(extra_size? &extra[0]:0, extra_size, mpi_treenode, split, 0, mpiComm(), &status);
      comm_pattern.extra_sizes.push_back(extra_size);
    }
    else
      comm_pattern.extra_sizes.push_back(0);

    comm_pattern.max_extra_size = max(int(comm_pattern.max_extra_size),extra_size);
    ...
    comm_pattern.merging_srcs.push_back(vector<merge_source>());
    comm_pattern.merging_types.push_back(vector<merge_type>());
    ...
    while(true)
    {
      // find "next" octant to push to new_l
      bool tmp_oct_empty = true;
      ot::TreeNode tmp_oct; 
      merge_source msrc;
	...
	msrc = QUEUE;
      ...
      if ( recv_ptr<recvd.size() && ( tmp_oct_empty || tmp_oct > recvd[recv_ptr] ) )
      {
	tmp_oct_empty = false;
	tmp_oct = recvd[recv_ptr];
	pp = &recv_ptr;
	msrc = RECVD;
      }

      if ( extra_ptr<extra.size() && ( tmp_oct_empty || tmp_oct > extra[extra_ptr] ) )
      {
	tmp_oct_empty = false;
	tmp_oct = extra[extra_ptr];
	pp = &extra_ptr;
	msrc = EXTRA;
      }
	...
	break;  // merging done
      ...
      comm_pattern.merging_srcs.back().push_back(msrc);

      if ( !new_l->empty() && tmp_oct == new_l->back() )  // same octant as previously pushed to new_l; we need to sum up densities
	comm_pattern.merging_types.back().push_back(ADD);
      else if ( pp == &recv_ptr || pp==&extra_ptr || ParInsLayerIntersectsRange(tmp_oct,q1,q2) )
	// (for received octants we do not need to check whether we should keep them -- we KNOW we should
      {
	assert( new_l->empty() || tmp_oct > new_l->back() );
	new_l->push_back(tmp_oct);
	comm_pattern.merging_types.back().push_back(INSERT);
      }
      else
	comm_pattern.merging_types.back().push_back(DROP);
    ...
    delete l;
    l=new_l;
    comm_pattern.max_queue_size = max(comm_pattern.max_queue_size,l->size());
  ...
  // some received octants may already be present in the tree, then do nothing
  ...
  comm_pattern.usr_octants.resize(l->size());
  for (size_t i=0; i<l->size(); i++)
  {
    int level = (*l)[i].getLevel();
    vector<int> coord(3);
    coord[0] = ((*l)[i].getX())>>( maxLevel()-level );
    coord[1] = ((*l)[i].getY())>>( maxLevel()-level );
    coord[2] = ((*l)[i].getZ())>>( maxLevel()-level );

    int q= 0;  // we start from root octant
    while(true)
    {
#ifdef DEBUG_LET
      assert(_nodeVec[q].depth() <= level);
#endif
      if (_nodeVec[q].depth() == level)
      {
	// at this point "_nodeVec[q]" should be same octant as "*l"
	// log q (to be coded later)
	_nodeVec[q].tag() |= LET_SRCENODE;
	comm_pattern.usr_octants[i]=q;
	break;
      }
	...
	unsigned idx[3];
	for(int j=0; j<3; j++)
	  idx[j]=( coord[j] >> (level - _nodeVec[q].depth() - 1) ) & 1; 
...
/**
 * @author Ilya Lashuk
 * @brief This function implemements the exchange of leaf octants data between processes, so that for each leaf octant the octant's data (see detailed description) is sent from the owner to all contributors and all users.
 *
 * Octant's data is: integer coordinates of octant and several indices: glbSrcExaBeg, glbSrcExaNum.
 * Flag LET_SRCENODE is also exchanged
 *
 * Algorithm for exchange is the following: owner of octant "o" sends "o" to all processes, whos areas intersect with the insulation layer of parent of "o". This is suboptimal in a sense that an octant can get to processors which do not touch it (and do not need exact densities). Such octants will be essentially discarded by receiving processor. Balancing the finest (linear) octree is recommended to alleviate this problem.
 */
#undef __FUNCT__
#define __FUNCT__ "Let3d_MPI::ExchangeOctantsNew"
void Let3d_MPI::ExchangeLeafOctants()
{
  ...
  for (size_t q=0; q<_nodeVec.size(); q++)
    if ( terminal(q) &&  _nodeVec[q].tag() & LET_OWNRNODE )
    {
      assert(_nodeVec[q].tag() & LET_OWNRNODE); // leaf can have at most one contributor, which should also be an owner
	...
	for (vector<ot::TreeNode>::const_iterator cpu_it=first_cpu; cpu_it<past_last_cpu; cpu_it++)
	  if (domain_overlaps[cpu_it-first_cpu])
	    // mark q to be sent to processor cpu_it
	    indToSend[cpu_it-procMins.begin()].push_back(q);
      }
    }
 ...
 // calculate send and recv displacements and total size to send and receive, reserve memory
  ...
  // print number of outgoing and incoming octants; the same octant sent to different processes is counted multiple times
  {
    ...
    if(!mpiRank())
      cout<<"ExchangeOctantsNew: number of outgoing octants (with repetitions)  min="<<minNumOut<<" max="<<maxNumOut<<endl;

    PetscInt locNumIn = totalRecvSize;
    PetscInt maxNumIn, minNumIn;
    MPI_Reduce (&locNumIn, &maxNumIn, 1, MPIU_INT, MPI_MAX, 0, mpiComm() );
    MPI_Reduce (&locNumIn, &minNumIn, 1, MPIU_INT, MPI_MIN, 0, mpiComm() );
    if(!mpiRank())
      cout<<"ExchangeOctantsNew: number of incoming octants, min="<<minNumIn<<" max="<<maxNumIn<<endl;
  }
...
int Let3d_MPI::srcData(bool printStatistics)
{
  ...
  /* based on the owner info, glbSrcExaBeg, glbSrcExaNum  */
  // also set global parameter _glbGlbSrcExaCnt
  // -------- assign _glbGlbSrcNodeCnt  --------------	 
  // (_glbGlbSrcNodeCnt might no longer be necessary,  but we'll keep it for now)
  ...
  {
    // "poison memory"
    // among other things this guarantees that all octants which are non-leaves in global tree will have -1 in fields "glbSrcExaNum" and "glbSrcExaBeg" (after "ExchangeLeafOctants")
