...
int FMM3d_MPI::SrcEqu2TrgChk_dgemv (const DblNumMat& srcPos, const DblNumMat& srcNor, const DblNumMat& trgPos, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense)
{
  if (fuse_dense)
    _knl.density2potential(srcPos, srcDen, srcNor, trgPos, trgVal);
  else
  {
    int TMAX = 1024;
    ...
    if(trgPos.n()<=TMAX) {
      ...
      iC( _knl.buildKnlIntCtx(srcPos, srcNor, trgPos, tmp) );
	...
	iC( _knl.buildKnlIntCtx(srcPos, srcNor, tps, tmp) );
...
int FMM3d_MPI::SrcEqu2UpwChk_dgemv(const DblNumMat& srcPos, const DblNumMat& srcNor, Point3 trgCtr, double trgRad, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense)
{
  ...
  if (fuse_dense)
    _knl_s2c.density2potential(srcPos, srcDen, srcNor, trgPos, trgVal);
  else
  {
    int M = trgPos.n() * _knl_s2c.trgDOF();
    int N = srcPos.n() * _knl_s2c.srcDOF();
    DblNumMat tmp(M,N);
    iC( _knl_s2c.buildKnlIntCtx(srcPos, srcNor, trgPos, tmp) );
    /* Compute 1.0*tmp*srcDen + 1.0*trgVal */
    iC( dgemv(1.0, tmp, srcDen, 1.0, trgVal) );
  }
  return(0);
...
int FMM3d_MPI::SrcEqu2DwnChk_dgemv(const DblNumMat& srcPos, const DblNumMat& srcNor, Point3 trgCtr, double trgRad, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense)
{
...
int FMM3d_MPI::DwnEqu2TrgChk_dgemv(Point3 srcCtr, double srcRad, const DblNumMat& trgPos, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense)
{
  ...
  if (fuse_dense) 
    _knl_e2t.density2potential(srcPos, srcDen, srcPos, trgPos, trgVal);
  else
  {
    /* Target Max = 1024 */
    int TMAX = 1024;
    ...
    if(trgPos.n()<=TMAX) {
      int M = trgPos.n() * _knl_e2t.trgDOF();
      int N = srcPos.n() * _knl_e2t.srcDOF();
      DblNumMat tmp(M,N);
      /* Build kernel multiplier context and store in tmp */
      iC( _knl_e2t.buildKnlIntCtx(srcPos, srcPos, trgPos, tmp) );
      /* Compute 1.0*tmp*srcDen + 1.0*trgVal */
      iC( dgemv(1.0, tmp, srcDen, 1.0, trgVal) );
    }
	...
	int end = min((r+1)*TMAX, trgPos.n());
	int num = end-stt;
	int M = num * _knl_e2t.trgDOF();
	int N = srcPos.n() * _knl_e2t.srcDOF();
	/* Target positions matrix */
	DblNumMat tps(dim(), num, false, trgPos.data() + stt*dim());
	/* Target values vector */
	DblNumVec tvl(num*_knl_e2t.trgDOF(), false, trgVal.data() + stt*_knl_e2t.trgDOF());
	DblNumMat tmp(M, N);
	/* Build multiplier context */
	iC( _knl_e2t.buildKnlIntCtx(srcPos, srcPos, tps, tmp) );
	/* Compute 1.0*tmp*srcDen + 1.0*tvl */
	iC( dgemv(1.0, tmp, srcDen, 1.0, tvl) );
      }
    }
...
int FMM3d_MPI::UpwEqu2TrgChk_dgemv(Point3 srcCtr, double srcRad, const DblNumMat& trgPos, const DblNumVec& srcDen, DblNumVec& trgVal, int fuse_dense)
{
