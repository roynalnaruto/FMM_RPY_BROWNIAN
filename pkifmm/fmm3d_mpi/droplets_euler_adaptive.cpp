...
int eval_rhs(Vec & x, Vec & ids, Vec & k1, Kernel3d_MPI & knl, double eps, bool redistr, double & delta_x )
  ...
  // if redistr is true, "x" is redistributed; otherwise not;  in any case layout of k1 will be compatible with that of "x"
{
  ...
  double scale_factor = (1-1e-10)/(max_coord-min_coord);
  double shift = -min_coord ;
  VecShift(xx, shift );
  ...
  delta_x = 1/scale_factor * ldexp(2.0, - fmm->let()->max_leaf_level); 
  PetscLogStagePop();
  ...
  if(redistr && ids)
  {
    PetscLogEventBegin(droplets_id_redistr,0,0,0,0);
    Vec new_ids;
    vector<PetscInt>  & newSrcGlobalIndices = fmm->let()->newSrcGlobalIndices;
    ...
    PetscLogEventEnd(droplets_id_redistr,0,0,0,0);
  }
...
//   StageLogSetVisible(CurrentStageLog, CurrentStage, PETSC_FALSE);
  ...
  if (!mpirank)
    cout<<"Using particle size: "<<eps<<" , time step: adaptive, number of iterations: "<<num_iter<<" , write fraction: "<<write_fraction<<endl;
...
//   }
  ...
  for (int i=1; i<=num_iter; i++)
  {
    ...
    double delta_x;
    eval_rhs(x, ids, k1, knl, eps, !preserve_order, delta_x);   // VecCreate(k1) is called inside, so destroy k1 outside


    // compute h adaptively,  based on  max(abs(k1)), and deepest tree level
    double k_norm;
    VecNorm(k1,NORM_INFINITY,&k_norm);
    h = 2*delta_x/k_norm;

    if (0==mpirank)
      cout<<"delta_x="<<delta_x<<" RHS inf-norm="<<k_norm<<" Using h= "<<h<<endl;
