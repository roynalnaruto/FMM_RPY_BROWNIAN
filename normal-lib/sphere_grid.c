...
double *sphere_icos1_points ( int factor, int node_num , double R )
...
*/
  ...
  for ( point = 0; point < point_num; point++ )
  {
    for ( dim = 0; dim < 3; dim++ )
    {
      node_xyz[dim+node*3] = R * point_coord[dim+point*3];
    }
    node = node + 1;
  }
/*
...
*/
  for ( edge = 0; edge < edge_num; edge++ )
  {
    ...
    for ( f = 1; f < factor; f++ )
    {
      ...
      for ( dim = 0; dim < 3; dim++ )
      {
        node_xyz[dim+node*3] = R * node_xyz[dim+node*3] / node_norm;
      }
      node = node + 1;
    }
  }
